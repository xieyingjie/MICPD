...
def Change_Based_communityAndNode(inter_prob, intra_prob, alpha, incrementintra, incrementintre):
    cps = [16, 34, 61, 87]
    fname = "hy0.1" + str(inter_prob) + "_" + str(intra_prob) + "_" + str(alpha) + ".txt"
    ...
    sizes_3 = [62, 62, 62, 62, 63, 63, 63, 63]
            ...
            if t == 16:
                ...
                for i in range(len(copy_probs)):
                    for j in range(len(copy_probs[0])):
                        if j == i:
                            copy_probs[i][i] = copy_probs[i][i] + 0.1
                G_t = SBM_snapshot(G_t, 0.6, sizes, np.asarray(copy_probs))
                probs = copy_probs
                ...
                G_t = SBM_snapshot(G_t, 0.2, sizes, np.asarray(copy_probs))
                probs = copy_probs
                G_times.append(G_t)
    ...
    for G in G_times:
        aj = np.array(nx.adjacency_matrix(G).todense())
        aj1 = np.array(nx.adjacency_matrix(G_times[ii]).todense())
        res = list(np.array(aj==aj1).reshape(-1,))
        matches = 0
        for b in res:
            if b:
             matches +=1
