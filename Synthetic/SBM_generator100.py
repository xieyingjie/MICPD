...
def Change_Based_communityAndNode(inter_prob, intra_prob, alpha, incrementintra, incrementintre):
    cps = [16, 21, 34, 46, 61, 87, 95]
    fname = "communityAndNoleCP_" + str(inter_prob) + "_" + str(intra_prob) + "_" + str(alpha) + ".txt"
    ...
    for t in range(maxt):
        if (t in cps):
            if t == 61:
                print('community and node change')
                ...
                for i in range(len(copy_probs)):
                    for j in range(len(copy_probs[0])):
                        if j == i:
                            copy_probs[i][i] = copy_probs[i][i] + incrementintra
                        if (copy_probs[i][j] < intra_prob):
                            copy_probs[i][j] = copy_probs[i][j] + 0.05
                G_t = SBM_snapshot(G_t, 0.9, sizes, np.asarray(copy_probs))
                probs = copy_probs
                G_times.append(G_t)
                ...
                #     for j in range(len(copy_probs[0])):
                #         if (copy_probs[i][j] < intra_prob):
                #             copy_probs[i][j] = copy_probs[i][j] + incrementintra
                ...
                for i in range(len(copy_probs)):
                    for j in range(len(copy_probs[0])):
                        if (j != i):
                            copy_probs[j][i] = copy_probs[j][i] + 0.05
                ...
                #
                #         if j != i:
                #             copy_probs[i][i] = copy_probs[i][i] + 0.05
                ...
                print('node and network changes')
                copy_probs = copy.deepcopy(probs)
                for i in range(len(copy_probs)):
                    for j in range(len(copy_probs[0])):
                        if j == i:
                            copy_probs[i][i] = copy_probs[i][i] - 0.2
                        if (j != i):
                            copy_probs[i][j] = copy_probs[i][j] - 0.05
                ...
                sizes = list_sizes[list_idx]
                probs = list_probs[list_idx]
                G_t = SBM_snapshot(G_t, 0.9, sizes, probs)
                G_times.append(G_t)
